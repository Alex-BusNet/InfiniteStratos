...
import com.sparta.is.utils.DamageSourceHelper;
import com.sparta.is.utils.EnergyHelper;
...
public class ItemElucidator extends ItemISMelee
{
    ...
    {
        super(Names.Weapons.ELUCIDATOR, "elucidator");
        maxEnergy = 50000;
    ...
    public void addInformation(ItemStack itemStack, EntityPlayer entityPlayer, List list, boolean par4)
    {
        ...
        list.add(" ");
        list.add(StringHelper.ORANGE + energyPerUse + " " + StringHelper.localize("info.is.tool.energyPerUse") + StringHelper.END);
    ...
    public EnumActionResult onItemUseFirst(EntityPlayer player, World world, BlockPos pos, EnumFacing side, float hitX, float hitY, float hitZ, EnumHand hand)
    {
        if(player.getItemStackFromSlot(EntityEquipmentSlot.CHEST) != ItemStack.EMPTY)
        {
            ...
            {
                if(((UnitKuroAkiko) itemStack.getItem()).getState() == 0)
                {
    ...
    public boolean hitEntity(ItemStack itemStack, EntityLivingBase entity, EntityLivingBase player)
    {
        ...
        if(entityPlayer.getItemStackFromSlot(EntityEquipmentSlot.CHEST) != ItemStack.EMPTY)
        {
            ...
            {
                if(((UnitKuroAkiko) item).getState() == 0)
                {
                    return true;
                }
            }
        ...
        if(!displayOnce)
        {
            ...
            if ( entityPlayer.getItemStackFromSlot(EntityEquipmentSlot.CHEST) != ItemStack.EMPTY )
            {
                ...
                if ( entityPlayer.getItemStackFromSlot(EntityEquipmentSlot.CHEST).getItem() instanceof UnitKuroAkiko )
                {
                    ...
                    if ( ((UnitKuroAkiko) unit).getState() == 0 )
                    {
