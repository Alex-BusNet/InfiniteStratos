...
public class ItemYukihiraNigata extends ItemISMelee implements IOwnable, IKeyBound, IEnergy
{
    ...
    private static final String[] VARIANTS = {"normal", "reiraku"};
    ...
    public EnumActionResult onItemUseFirst(EntityPlayer player, World world, BlockPos pos, EnumFacing side, float hitX, float hitY, float hitZ, EnumHand hand)
    {
        if(player.getItemStackFromSlot(EntityEquipmentSlot.CHEST) != ItemStack.EMPTY)
        {
            ...
            {
                if(((UnitByakushiki) itemStack.getItem()).getState().getMeta() == 0)
                {
    ...
    public boolean hitEntity(ItemStack itemStack, EntityLivingBase entity, EntityLivingBase player)
    {
        ...
        if(entityPlayer.getItemStackFromSlot(EntityEquipmentSlot.CHEST) != ItemStack.EMPTY)
        {
            ...
            {
                if(((UnitByakushiki) item).getState().getMeta() == 0)
                {
                    return true;
                }
        ...
        if(!entityPlayer.capabilities.isCreativeMode || useEnergy(itemStack, false) == getEnergyPerUse(itemStack))
        {
            ...
            {
                entity.attackEntityFrom(DamageSourceHelper.sword, Materials.Weapons.IS_SWORD.getAttackDamage());
            }
            else
            {
                if(oneOff == 1)
    ...
    public void addInformation(ItemStack itemStack, World worldIn, List<String> toolTip, ITooltipFlag ttFlag)
    {
        ...
        else if(localOneOff == 1)
        {
            ...
            toolTip.add(StringHelper.RED + "Consumes lots of energy very quickly" + StringHelper.END);
        }

        toolTip.add(" ");
        toolTip.add(StringHelper.ORANGE + getEnergyPerUse(itemStack) + " " + StringHelper.localize("info.is.tool.energyPerUse") + StringHelper.END);
        ...
        if(!displayOnce)
        {
            ...
            if ( entityPlayer.getItemStackFromSlot(EntityEquipmentSlot.CHEST) != ItemStack.EMPTY )
            {
                ...
                if ( isUnit.getItem() instanceof UnitByakushiki)
                {
                    ...
                    if ( ((UnitByakushiki) isUnit.getItem()).getState().getMeta() == 0 )
                    {
