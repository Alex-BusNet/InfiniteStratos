...
public class DriveablePart
{
    ...
    public void update(EntityUnitController driveable)
    {
        if(fireTime > 0)
            fireTime--;
        if(fireTime == 0)
    ...
    public float smashIntoGround(EntityUnitController driveable, float damage)
    {
    ...
    public Raytracer.DriveableHit rayTrace(EntityUnitController driveable, Vector3f origin, Vector3f motion)
    {
		...
		*/
        ...
        if(motion.x != 0F)
        {
            if(origin.x < box.x) //Check face x = o.x
            {
                ...
                if(intersectY >= box.y && intersectY <= box.y + box.height && intersectZ >= box.z && intersectZ <= box.z + box.depth)
                    return new Raytracer.DriveableHit(driveable, type, intersectTime);
            }
            else if(origin.x > box.x + box.width) //Check face x = o.x + d.x
            {
                float intersectTime = (box.x + box.width - origin.x) / motion.x;
                float intersectY = origin.y + motion.y * intersectTime;
                float intersectZ = origin.z + motion.z * intersectTime;
                if(intersectY >= box.y && intersectY <= box.y + box.height && intersectZ >= box.z && intersectZ <= box.z + box.depth)
                    return new Raytracer.DriveableHit(driveable, type, intersectTime);
        ...
        if(motion.z != 0F)
        {
            if(origin.z < box.z) //Check face z = box.z
            {
                ...
                float intersectY = origin.y + motion.y * intersectTime;
                if(intersectX >= box.x && intersectX <= box.x + box.width && intersectY >= box.y && intersectY <= box.y + box.height)
                    return new Raytracer.DriveableHit(driveable, type, intersectTime);
            }
            else if(origin.z > box.z + box.depth) //Check face z = box.z + box.d
            {
                float intersectTime = (box.z + box.depth - origin.z) / motion.z;
                float intersectX = origin.x + motion.x * intersectTime;
                float intersectY = origin.y + motion.y * intersectTime;
                if(intersectX >= box.x && intersectX <= box.x + box.width && intersectY >= box.y && intersectY <= box.y + box.height)
                    return new Raytracer.DriveableHit(driveable, type, intersectTime);
        ...
        if(motion.y != 0F)
        {
            if(origin.y < box.y) //Check face y = o.y
            {
                ...
                float intersectX = origin.x + motion.x * intersectTime;
                float intersectZ = origin.z + motion.z * intersectTime;
                if(intersectX >= box.x && intersectX <= box.x + box.width && intersectZ >= box.z && intersectZ <= box.z + box.depth)
                    return new Raytracer.DriveableHit(driveable, type, intersectTime);
            }
            else if(origin.y > box.y + box.height) //Check face x = box.y + box.h
            {
                float intersectTime = (box.y + box.height - origin.y) / motion.y;
                float intersectX = origin.x + motion.x * intersectTime;
                float intersectZ = origin.z + motion.z * intersectTime;
                if(intersectX >= box.x && intersectX <= box.x + box.width && intersectZ >= box.z && intersectZ <= box.z + box.depth)
                    return new Raytracer.DriveableHit(driveable, type, intersectTime);
    ...
    {
        //Perform damage code
        health -= damage * type.damageVsUnits;
