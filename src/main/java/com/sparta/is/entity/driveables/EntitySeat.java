...
import com.sparta.is.entity.EntityUnitController;
...
public class EntitySeat extends Entity implements IControllable, IEntityAdditionalSpawnData
{
    ...
    private int seatID;
    public EntityUnitController driveable;
    ...
    public EntitySeat(World world, EntityUnitController d, int id)
    {
        this(world);
        driveable = d;
    ...
    public void onUpdate()
    {
        ...
        {
            driveable = (EntityUnitController)world.getEntityByID(driveableID);
            if(driveable == null)
                return;
            foundDriveable = true;
...
//        }
        ...
        boolean isThePlayer = getRidingEntity() instanceof EntityPlayer && Minecraft.getMinecraft().player.equals(getRidingEntity());
        ...
        if(world.isRemote)
        {
            if(driver && isThePlayer && driveable.hasMouseControlMode())
            {
                looking = new RotatedAxes();
                playerLooking = new RotatedAxes();
            }
        ...
        if(getRidingEntity() instanceof EntityPlayer && shooting)
            pressKey(9, (EntityPlayer)getRidingEntity());
    ...
    public void updatePosition()
    {
        ...
        if(getRidingEntity() != null)
        {
            ...
            Vector3f yOff = driveable.axes.findLocalVectorGlobally(new Vector3f(0, getRidingEntity().getEyeHeight() * 3 / 4, 0));
            Vec3d yOffset = new Vec3d(yOff.x, yOff.y, yOff.z);
            yOffset.subtract(0, getRidingEntity().getEyeHeight(), 0);
            ...
            getRidingEntity().lastTickPosX = getRidingEntity().prevPosX = prevPlayerPosX;
            getRidingEntity().lastTickPosY = getRidingEntity().prevPosY = prevPlayerPosY;
            getRidingEntity().lastTickPosZ = getRidingEntity().prevPosZ = prevPlayerPosZ;
            getRidingEntity().setPosition(playerPosX, playerPosY, playerPosZ);
            ...
            {
                getRidingEntity().prevRotationYaw = prevPlayerYaw;
                getRidingEntity().prevRotationPitch = prevPlayerPitch;

                getRidingEntity().rotationYaw = playerYaw;
                getRidingEntity().rotationPitch = playerPitch;
            }
...
//    }
    ...
    public void onMouseMoved(int deltaX, int deltaY)
    {
        ...
        if(!driver || ! ClientSideUtils.controlModeMouse || !driveable.hasMouseControlMode())
        {
            float lookSpeed = 4F;
    ...
    public boolean pressKey(int key, EntityPlayer player)
    {
        ...
        //Exit key pressed
        if(key == 6 && getRidingEntity() != null)
            getRidingEntity().startRiding(null);

        if(key == 9) //Shoot
        {
            //Get the gun from the plane type and the ammo from the data
            EqualizerRangeType gun = seatInfo.gunType;

            minigunSpeed += 0.1F;

            if(gun != null && minigunSpeed > 2F)
            {
                if(gunDelay <= 0)
                {
                    ...
                    if(gun != null && bulletItemStack != null && bulletItemStack.getItem() instanceof ItemShootable)
                    {
                        ...
                        {
                            //Gun origin
                            Vector3f gunOrigin = Vector3f.add(driveable.axes.findLocalVectorGlobally(seatInfo.gunOrigin), new Vector3f((float)driveable.posX, (float)driveable.posY, (float)driveable.posZ), null);
...
//                            }
                            ...
                            if(damage + 1 == bulletItemStack.getMaxDamage())
                            {
                                ...
                                if(!((EntityPlayer)getRidingEntity()).capabilities.isCreativeMode)
                                    driveable.getDriveableData().decrStackSize(3 + seatID, 1);
                            }
                            //Reset the shoot delay
                            gunDelay = gun.delayBetweenShots;
                        }
                    }
                }
            }
    ...
    public boolean interactFirst(EntityPlayer entityplayer) //interact : change back when Forge updates
    {
        ...
        ItemStack currentItem = entityplayer.getActiveItemStack();
        if(currentItem != ItemStack.EMPTY && currentItem.getItem() instanceof ItemLead )
        {
            if(getRidingEntity() != null && getRidingEntity() instanceof EntityLiving && !(getRidingEntity() instanceof EntityPlayer))
            {
                EntityLiving mob = (EntityLiving)getRidingEntity();
                getRidingEntity().startRiding(null);
        ...
        if(getRidingEntity() == null && !driveable.getDriveableData().core.isAIChip)
        {
    ...
    public void readSpawnData(ByteBuf data)
    {
        ...
        if(world.getEntityByID(driveableID) instanceof EntityUnitController)
            driveable = (EntityUnitController)world.getEntityByID(driveableID);
        seatID = data.readInt();
        driver = seatID == 0;
        if(driveable != null)
