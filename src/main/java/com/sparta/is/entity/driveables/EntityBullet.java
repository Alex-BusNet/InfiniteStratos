...
import com.sparta.is.entity.EntityDamageSourceGun;
import com.sparta.is.entity.EntityISUnit;
import com.sparta.is.entity.EntityShootable;
...
public class EntityBullet extends EntityShootable implements IEntityAdditionalSpawnData
{
    ...
    private void getLockOnTarget() {
        if (type.canLockOn) {
            ...
            for (Object obj : world.loadedEntityList) {
                ...
                if ((type.canLockOn && entity instanceof EntityISUnit)) {
                    Vector3f relPosVec = new Vector3f((float)entity.posX - (float)posX, (float)entity.posY - (float)posY, (float)entity.posZ - (float)posZ);
    ...
    public static boolean OnHit(World world, Vector3f origin, Vector3f hit, Entity shooter, InfoType shotFrom, ShootableType shootableType, EntityBullet bullet, float damage, Raytracer.BulletHit bulletHit)
    {
        if(!(shootableType instanceof BulletType))
        {
            LogHelper.info(LogHelper.MOD_MARKER, "Tried to fire grenade instantly");
            return true;
        }
        if(world == null || origin == null || hit == null || shooter == null || shotFrom == null || shootableType == null)
        {
            LogHelper.info(LogHelper.MOD_MARKER, "Something was null");
            return true;
        }
        ...
        if(bulletHit instanceof Raytracer.DriveableHit )
        {
            Raytracer.DriveableHit driveableHit = (Raytracer.DriveableHit)bulletHit;
...
//                world.spawnEntityInWorld(new EntityDebugDot(world, hit, 1000, 0F, 0F, 1F));
        ...
        else if(bulletHit instanceof Raytracer.PlayerBulletHit )
        {
            Raytracer.PlayerBulletHit playerHit = (Raytracer.PlayerBulletHit)bulletHit;
...
//                world.spawnEntityInWorld(new EntityDebugDot(world, hit, 1000, 1F, 0F, 0F));
        ...
        else if(bulletHit instanceof Raytracer.EntityHit )
        {
            Raytracer.EntityHit entityHit = (Raytracer.EntityHit)bulletHit;
...
//                    world.spawnEntityInWorld(new EntityDebugDot(world, hit, 1000, 1F, 1F, 0F));
        ...
        else if(bulletHit instanceof Raytracer.BlockHit )
        {
            Raytracer.BlockHit blockHit = (Raytracer.BlockHit)bulletHit;
...
//        }
        ...
        if(!world.isRemote)
        {
            List<Raytracer.BulletHit> hits = Raytracer.Raytrace(world, owner, ticksInAir > 20, this, origin, motion,
                    pingOfShooter);

            // We hit something
            if (!hits.isEmpty())
            {
                for (Raytracer.BulletHit bulletHit : hits)
                {
        ...
        if (lockedOnTo != null) {
            ...
            Vector3f relPosVec = new Vector3f((float)(lockedOnTo.posX - posX), (float)(lockedOnTo.posY - posY), (float)(lockedOnTo.posZ - posZ));
...
//        }
    ...
    private void spawnParticles() {
        ...
        float spread = 0.1F;
        for (int i = 0; i < 10; i++) {
            Particle particle = ClientSideUtils.getParticle(type.trailParticleType, world,
...
//                particle.renderDistanceWeight = 100D;
    ...
    private boolean isPartOfOwner(Entity entity) {
        if (owner == null)
            return false;
        if (entity == owner || entity == owner.getRidingEntity() || entity.equals(owner.getRidingEntity()))
            return true;
        if (owner instanceof EntityPlayer)
        {
            if ( PlayerEventHandler.getPlayerData((EntityPlayer) owner, world.isRemote ? Side.CLIENT : Side.SERVER) == null)
                return false;
        }
    ...
    public void setDead()
    {
        ...
        OnDetonate(world, new Vector3f((float)posX, (float)posY, (float)posZ), owner, this, firedFrom, type);
    }
...
//        }
        ...
        if (bulletType.dropItemOnHit != null)
        {
            ...
            if (dropStack != ItemStack.EMPTY && dropStack.getCount() != 0 && dropStack.getItem() != Item.getItemFromBlock(Blocks.AIR))
            {
    ...
    public void readSpawnData(ByteBuf data) {
        try {
            ...
            LogHelper.info(LogHelper.MOD_MARKER, "Failed to read bullet owner from server.");
            super.setDead();
            e.printStackTrace();
        }
    }
