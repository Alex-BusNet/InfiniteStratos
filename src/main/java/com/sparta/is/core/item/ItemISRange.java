...
import com.sparta.is.core.utils.helpers.InventoryHelper;
        ...
        if(type.usableByPlayers)
        {
            ...
            if(needsToReload)
            {
                if(Reload(gunstack, world, player, player.inventory, isOffHand, hasOffHand, false, player.capabilities.isCreativeMode))
                {
                    ...
                    int pumpDelay = type.model == null ? 0 : 0; //type.model.pumpDelayAfterReload;
                    int pumpTime = type.model == null ? 1 : 1;//type.model.pumpTime;
            ...
            else if(shouldShootThisTick)
            {
                ...
                while(shootTime <= 0.0f)
                {
                    ...
                    if(type.bulletSpeed == 0.0f)
                    {
                        ...
                        {
                            Vector3f rayTraceOrigin = new Vector3f((float)player.getPositionEyes(0.0f).x, (float)player.getPositionEyes(0.0f).y, (float)player.getPositionEyes(0.0f).z);
                            Vector3f rayTraceDirection = new Vector3f((float)player.getLookVec().x, (float)player.getLookVec().y, (float)player.getLookVec().z);
...
//                            }
                    ...
                    {
                        ShotData shotData = new ShotData.SpawnEntityShotData(gunSlot, type, shootableType, player, new Vector3f((float)player.getLookVec().z, (float)player.getLookVec().y, (float)player.getLookVec().z));
                        shotsFiredClient.add(shotData);
                    }
                    ...
                    int pumpDelay = type.model == null ? 0 : 0;//type.model.pumpDelay;
                    int pumpTime = type.model == null ? 1 : 1;//type.model.pumpTime;
...
//                }
                ...
                for (int k = 0; k < type.numBullets * bullet.numBullets; k++)
                {
                    ...
                    ((ItemShootable)bulletStack.getItem()).Shoot(world,
                            ...
                            new Vec3d(player.getLookVec().x, player.getLookVec().y, player.getLookVec().z),
            ...
            if(hitData instanceof Raytracer.BlockHit )
            {
                ...
                if(blockState != null)
                {
                    for(int i = 0; i < 2; i++)
                    {
                        ...
                        fx.setPosition((double)normal.getX() * scale + world.rand.nextGaussian() * 0.025d,
                        (double)normal.getY() * scale + world.rand.nextGaussian() * 0.025d,
                        (double)normal.getZ() * scale + world.rand.nextGaussian() * 0.025d);
