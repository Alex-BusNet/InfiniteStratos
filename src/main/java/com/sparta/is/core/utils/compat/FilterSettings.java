package com.sparta.is.core.utils.compat;

import com.sparta.is.core.inventory.ContainerFilter;
...
//                        ItemDrill.loadSlotsFromNBT(inv, slot);
                        for(int k = 0; k < inv.getSlots(); k++){
                            ...
                            if(ItemStackUtils.isValid(filterSlot) && areEqualEnough(filterSlot, stack, meta, nbt, mod, oredict)){
                                return whitelist;
                            }
                        }
                    }
    ...
    public boolean needsCheck(){
        for(int i = 0; i < this.filterInventory.getSlots(); i++){
            if( ItemStackUtils.isValid(this.filterInventory.getStackInSlot(i))){
                return true;
            }
        }
