...
public class InventoryHelper
{
        ...
        else
        {
            try
            {
                ...
                if (stack.isItemDamaged())
                {
                    ...
                    if (i >= 0)
                    {
                        ItemStack stackToAdd = new ItemStack(stack.getItem());
    ...
    public static int storeItemStack(IInventory inventory, ItemStack stack)
    {
        for (int i = 0; i < inventory.getSizeInventory(); ++i)
        {
            ...
            if (oldStack != ItemStack.EMPTY && oldStack.getItem() == stack.getItem() && oldStack.isStackable() && oldStack.getCount() < oldStack.getMaxStackSize() && oldStack.getCount() < inventory.getInventoryStackLimit() && (!oldStack.getHasSubtypes() || oldStack.getItemDamage() == stack.getItemDamage()) && ItemStack.areItemStackTagsEqual(oldStack, stack))
            {
                return i;
            }
        }
    ...
    public static int storePartialItemStack(IInventory inventory, ItemStack stack)
    {
        ...
        if(stack.getMaxStackSize() == 1)
        {
            ...
            else
            {
                ...
                {
                    inventory.setInventorySlotContents(k, new ItemStack(stack.getItem()));
                }
                return 0;
            }
        }
        else
        {
            ...
            else
            {
                ...
                {
                    j -= l;
                    oldStack.setCount(oldStack.getCount() + 1);
