...
import com.sparta.is.entity.driveables.EnumDriveablePart;
...
public class Raytracer
{
    public static List<BulletHit> Raytrace(World world, Entity playerToIgnore, boolean canHitSelf, Entity entityToIgnore, Vector3f origin, Vector3f motion, int pingOfShooter)
    {
        ...
        for(int i = 0; i < world.loadedEntityList.size(); i++)
        {
            ...
            //Get driveables
            if(obj instanceof EntityUnitController)
            {
                EntityUnitController driveable = (EntityUnitController)obj;
            ...
            if(shouldDoNormalHitDetect)
            {
                ...
                {
                    RayTraceResult mop = entity.getEntityBoundingBox().calculateIntercept(new Vec3d(origin.x, origin.y, origin.z), new Vec3d(origin.x + motion.x, origin.y + motion.y, origin.z + motion.z));
                    if(mop != null)
                    {
                        Vector3f hitPoint = new Vector3f((float)mop.hitVec.xCoord - origin.x, (float)mop.hitVec.yCoord - origin.y, (float)mop.hitVec.zCoord - origin.z);
                        float hitLambda = 1F;
                        if(motion.x != 0F)
        ...
        Vec3d posVec = new Vec3d(origin.x, origin.y, origin.z);
        Vec3d nextPosVec = new Vec3d(motion.getX(), motion.getY(), motion.getZ()).add(posVec);
        RayTraceResult hit = world.rayTraceBlocks(posVec, nextPosVec, false, true, true);

        posVec = new Vec3d(origin.x, origin.y, origin.z);
    ...
    public static Vector3f GetPlayerMuzzlePosition(EntityPlayer player, boolean isOffHand)
    {
        ...
        if(itemstack != ItemStack.EMPTY && itemstack.getItem() instanceof ItemISRange)
        {
            EqualizerRangeType gunType = ((ItemISRange)itemstack.getItem()).getType();
            AttachmentType barrelType = gunType.getBarrel(itemstack);

            return Vector3f.add(new Vector3f((float)player.posX, (float)player.posY, (float)player.posZ), snapshot.GetMuzzleLocation(gunType, barrelType, isOffHand), null);
        }

        return new Vector3f((float)player.getPositionEyes(0.0f).xCoord, (float)player.getPositionEyes(0.0f).yCoord, (float)player.getPositionEyes(0.0f).zCoord);
    }
    ...
    public static class DriveableHit extends BulletHit
    {
        ...
        public DriveableHit(EntityUnitController d, EnumDriveablePart p, float f)
        {
            super(f);
            part = p;
            driveable = d;
    ...
    public static BulletHit ReadFromBuffer(ByteBuf buffer)
    {
        ...
        switch(type)
        {
            ...
            case 2: // DriveableHit
            {
                float intersectTime = buffer.readFloat();
                Entity entity = GetEntityByID(buffer.readInt());
                if(entity instanceof EntityUnitController)
                {
                    return new DriveableHit((EntityUnitController)entity, EnumDriveablePart.values()[buffer.readByte()], intersectTime);
                }
                else
                {
                    LogHelper.info(LogHelper.MOD_MARKER, "Entity was not a driveable");
                    return null;
                }
            }
            case 3: // PlayerBulletHit
            {
                ...
                {
                    LogHelper.info(LogHelper.MOD_MARKER, "Entity was not a player");
                    return null;
                }
            }
        }
