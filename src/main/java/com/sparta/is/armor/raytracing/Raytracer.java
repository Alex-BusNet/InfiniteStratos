...
public class Raytracer
{
        ...
        for(int i = 0; i < world.loadedEntityList.size(); i++)
        {
            ...
            if(shouldDoNormalHitDetect)
            {
                        ...
                        && entity.getEntityBoundingBox() != null)
                {
                    ...
                    if(mop != null)
                    {
                        Vector3f hitPoint = new Vector3f((float)mop.hitVec.x - origin.x, (float)mop.hitVec.y - origin.y, (float)mop.hitVec.z - origin.z);
                        float hitLambda = 1F;
                        if(motion.x != 0F)
    ...
    public static Vector3f GetPlayerMuzzlePosition(EntityPlayer player, boolean isOffHand)
    {
        ...
        return new Vector3f((float)player.getPositionEyes(0.0f).x, (float)player.getPositionEyes(0.0f).y, (float)player.getPositionEyes(0.0f).z);
    }
