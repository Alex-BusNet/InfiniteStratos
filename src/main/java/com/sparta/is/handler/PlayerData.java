...
public class PlayerData
{
    ...
    public void clientTick(EntityPlayer player)
    {
        if(player.getActiveItemStack() == ItemStack.EMPTY
                ...
                || ((ItemISRange)player.getActiveItemStack().getItem()).getType().oneHanded
    ...
    public boolean isValidOffHandWeapon(EntityPlayer player, int slot)
    {
        ...
        if(stackInSlot == null)
            return false;
        if(stackInSlot.getItem() instanceof ItemISRange)
        {
            ItemISRange item = ((ItemISRange)stackInSlot.getItem());
    ...
    public void cycleOffHandItem(EntityPlayer player, int dWheel)
    {
        ...
        InfiniteStratos.instance.currentScope = null;

//        FlansMod.getPacketHandler().sendToServer(new PacketSelectOffHandGun(offHandGunSlot));
    }

    public void doMelee(EntityPlayer player, int meleeTime, EqualizerMeleeType type)
    {
