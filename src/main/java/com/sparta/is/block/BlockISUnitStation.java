...
import com.sparta.is.item.ItemUnitStation;
...
public class BlockISUnitStation extends BlockISContainerBase implements IInitializer, IModelRegister
{
    protected ArmorIS storedUnit;

    private static AxisAlignedBB BOTTOM_HALF_BB = new AxisAlignedBB(0.0D, 0.0D, 0.0D, 1.0D, 1.0D, 1.0D);

    public BlockISUnitStation()
    {
        super(Material.IRON, Names.Blocks.IS_UNIT_STATION);

        this.setUnlocalizedName(Names.Blocks.IS_UNIT_STATION);
    ...
    {
        return new TileEntityISUnitStation();
    }
...
//        }
        ...
        if(!world.isRemote)
        {
            TileEntityISUnitStation stand = (TileEntityISUnitStation)world.getTileEntity(pos);
            if(stand != null)
            {
    ...
    {
        this.setRegistryName(Names.Blocks.IS_UNIT_STATION);
        ModBlocks.registerTileEntity(this);

        ItemUnitStation itemStation = new ItemUnitStation(this);
        itemStation.setRegistryName(this.getRegistryName());
        ForgeRegistries.ITEMS.register(itemStation);

        unitStation = new ItemStack(this, 1, 0);
    ...
    {
        ModelLoader.setCustomModelResourceLocation(Item.getItemFromBlock(this), 0, new ModelResourceLocation(Reference.MOD_ID + ":" + Names.Blocks.IS_UNIT_STATION, "inventory"));
    }

    private static ItemStack unitStation;
}
